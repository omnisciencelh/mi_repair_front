...
<script>
import { WorkerAcceptOrder, WorkerSearchOrder, WorkerRepairSuccess, WorkerRepairFailed } from '@/api/comment/repairOrder'
    ...
    repairOver (id) {
      ...
      WorkerRepairSuccess(id)
        .then((data) => {
          ...
          this.$message.error('维修成功按钮失效')
          console.error('Error fetching data:', error)
        })
    },
    // 维修失败
    repairFail (id) {
      WorkerRepairFailed(id)
        .then((data) => {
          this.$message({
            message: '维修失败',
            type: 'success'
